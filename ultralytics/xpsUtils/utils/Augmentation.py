...
def small_object_oversampling(image, labels, aug_ops):
    ...
    for _ in range(Config.OVERSAMPLING_FACTOR):
        ...
        new_w = max(1, new_w)
        new_h = max(1, new_h)
        small_obj_scaled = cv2.resize(small_obj_pixel, (new_w, new_h), interpolation=cv2.INTER_LINEAR)
...
def synthesize_dense_small_objects(image, labels, aug_ops):
    ...
    for _ in range(Config.SYNTHESIS_NUM):
        ...
        new_w = int(obj_w * synth_scale)
        new_h = int(obj_h * synth_scale)
        new_w = max(1, new_w)
        new_h = max(1, new_h)
        small_obj_scaled = cv2.resize(small_obj_pixel, (new_w, new_h), interpolation=cv2.INTER_LINEAR)
    ...
    aug_ops.append("small_synthesis")  # 记录操作
    return new_image, new_labels, aug_ops
